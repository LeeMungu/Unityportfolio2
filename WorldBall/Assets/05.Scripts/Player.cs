...
public class Player : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            transform.localPosition =
                (transform.localPosition + transform.forward.normalized * m_Speed * Time.deltaTime).normalized * 11f;
        }
        //중력에 따른 캐릭터 회전
        RotateBody(ahorizontal,bvertical);

        //transform.rotation = Quaternion.Euler(0,m_rotationSpeed * ahorizontal, 0);
    ...
    void RotateBody(float ah,float vertical)
    {
       // float rateY = transform.rotation.y + m_rotationSpeed * ah;
        transform.up = transform.position.normalized;

        transform.Rotate(0f, ah * m_rotationSpeed * Time.deltaTime, 0f);
    }
